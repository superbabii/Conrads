...
const StyledTab = styled((props: StyledTabProps) => (
  ...
  "&.Mui-focusVisible": {
    backgroundColor: "rgba(113, 0, 16, 1)",
  },
}));
  ...
  return (
    <div className="work-card rounded-md flex">
      ...
      <div className="flex flex-col justify-center px-4 w-[200px] gap-y-4">
        <h3 className="text-sm text-maingray rounded-md">{productName}</h3>
        <div className="flex justify-between items-center">
          <p className="text-primary text-lg">${price}</p>
...
const ProductTabCard: React.FC<ProductTabCardProps> = ({
  productName,
  imageUrl,
  price,
  discount,
  ...
  return (
    <div className="relative work-card rounded-lg w-[250px]">
      ...
      <div className="absolute bottom-0 left-0 w-full bg-black bg-opacity-75 px-2 py-1 text-white flex justify-between items-center rounded-b-lg">
        ...
        {discount && <span className="text-lg">Discount: {discount}%</span>}
...
const Products = () => {
  ...
  const handleChange = (event: React.SyntheticEvent, newValue: number) => {
    ...
    setCurrentCategory(
      newValue === 0
        ? "All"
        : ["All", "Automotive", "AI", "Electronic", "Software"][newValue]
    );
  };
